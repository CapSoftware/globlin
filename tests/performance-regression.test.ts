...
import { measureTime, measureTimeSync, loadGloblin, formatDuration, formatSpeedup } from './utils.js'
...
describe('Performance Regression Suite', () => {
  ...
  async function benchmarkPattern(
    ...
    const variance = globlinTimes.reduce((sum, t) => sum + Math.pow(t - meanGloblinTime, 2), 0) / BENCHMARK_RUNS
  ...
  describe('Scoped Pattern Regressions', () => {
    it('level0/**/*.js must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('level0/**/*.js')
    ...
    it('**/level1/**/*.ts must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('**/level1/**/*.ts')
  ...
  describe('Option Impact Regressions', () => {
    it('dot: true must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('**/*.js', { dot: true })
    ...
    it('nocase: true must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('**/*.JS', { nocase: true })
    ...
    it('nodir: true must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('**/*', { nodir: true })
    ...
    it('absolute: true must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('**/*.js', { absolute: true })
  ...
  describe('Complex Pattern Regressions', () => {
    ...
    it('question mark patterns must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('**/level?/**/*.ts')
    ...
    it('brace expansion patterns must not regress', async () => {
      ...
      const { globTime, globlinTime, speedup, resultCount } = await benchmarkPattern('level{0,1}/**/*.js')
    ...
    it('all patterns must be at least as fast as glob', async () => {
      ...
      if (failedPatterns.length > 0) {
        ...
        for (const { pattern, speedup } of failedPatterns) {
          console.log(`    ${pattern}: ${formatSpeedup(speedup)} (need >= ${REGRESSION_THRESHOLDS.MIN_SPEEDUP}x)`)
        }
      }
...
describe('Regression Summary', () => {
  it('should report threshold configuration', () => {
    ...
    console.log(`  MIN_SPEEDUP: ${REGRESSION_THRESHOLDS.MIN_SPEEDUP}x (must never be slower than glob)`)
