...
describe.skipIf(isWindows)('Symlink Following', () => {
  ...
  beforeAll(async () => {
    ...
    fs.symlinkSync(
      path.join(fixtureDir, 'a', 'b'),
      path.join(fixtureDir, 'a', 'symlink')
    )
  })
  ...
  describe('follow: true', () => {
    ...
    it('globlin: should follow symlinks when follow: true', async () => {
      ...
      const results = await globlin.glob('a/**/*.txt', { cwd: fixtureDir, follow: true, posix: true })
      results.sort()
    ...
    it('should match glob behavior with follow: true', async () => {
      ...
      const globResults = new Set(await glob.glob('a/**/*.txt', { cwd: fixtureDir, follow: true, posix: true }))
      const globlinResults = new Set(await globlin.glob('a/**/*.txt', { cwd: fixtureDir, follow: true, posix: true }))
  ...
  describe('sync versions', () => {
    ...
    it('globSync should match sync glob behavior with follow: true', () => {
      ...
      const globResults = new Set(glob.globSync('a/**/*.txt', { cwd: fixtureDir, follow: true, posix: true }))
      const globlinResults = new Set(globlin.globSync('a/**/*.txt', { cwd: fixtureDir, follow: true, posix: true }))
...
describe.skipIf(isWindows)('Broken Symlinks', () => {
  ...
  beforeAll(async () => {
    ...
    fs.symlinkSync(
      'this-does-not-exist',
      path.join(fixtureDir, 'a', 'broken-link', 'link')
    )
  })
