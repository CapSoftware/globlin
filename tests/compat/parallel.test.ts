  ...
  describe('with other options', () => {
    ...
    it('parallel works with nodir: true', () => {
      const serialResults = new Set(
        globSync('**/*', { cwd: fixture, parallel: false, nodir: true })
      )
      const parallelResults = new Set(
        globSync('**/*', { cwd: fixture, parallel: true, nodir: true })
      )
...
describe('parallel option with large fixture', () => {
  ...
  it('parallel works with complex pattern on large fixture', () => {
    const serialResults = new Set(
      globSync('**/subdir/*.js', { cwd: largeFixture, parallel: false })
    )
    const parallelResults = new Set(
      globSync('**/subdir/*.js', { cwd: largeFixture, parallel: true })
    )
