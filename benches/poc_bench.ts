...
async function runPoCBenchmark(fixtureName: string): Promise<PatternResult[]> {
  ...
  for (const { name, pattern, description } of POC_PATTERNS) {
    ...
    // Run glob (reference)
    const globResult = runBenchmark('glob', () =>
      globOriginalSync(pattern, { cwd: fixture })
    );

    // Run fast-glob
    const fgResult = runBenchmark('fast-glob', () =>
      fg.sync(pattern, { cwd: fixture })
    );

    // Run globlin
    const globlinResult = runBenchmark('globlin', () =>
      globlinSync(pattern, { cwd: fixture })
    );

    // Verify results match
    const globSet = new Set(globResult.resultCount > 0 ? globOriginalSync(pattern, { cwd: fixture }) : []);
    const globlinSet = new Set(globlinResult.resultCount > 0 ? globlinSync(pattern, { cwd: fixture }) : []);
    ...
    // Print results
    console.log(`  glob:      ${formatTime(globResult.mean).padEnd(10)} (${globResult.resultCount} results)`);
    console.log(`  fast-glob: ${formatTime(fgResult.mean).padEnd(10)} (${fgResult.resultCount} results)`);
    console.log(`  globlin:   ${formatTime(globlinResult.mean).padEnd(10)} (${globlinResult.resultCount} results)`);
    console.log();
    ...
    if (!resultsMatch) {
      console.log(`  WARNING: Results don't match! glob=${globSet.size}, globlin=${globlinSet.size}`);
    }
...
function printSummary(results: PatternResult[]): void {
  ...
  // Calculate averages
  const avgGloblinVsGlob = results.reduce((acc, r) => acc + r.speedup.globlinVsGlob, 0) / results.length;
  const avgGloblinVsFg = results.reduce((acc, r) => acc + r.speedup.globlinVsFastGlob, 0) / results.length;
  ...
  if (minSpeedup >= 5) {
    ...
    console.log(`  STATUS: NEEDS IMPROVEMENT - ${((5 - minSpeedup) / 5 * 100).toFixed(0)}% below target`);
  }
  console.log('  ' + '='.repeat(66));
}
