...
async function main() {
  ...
  const patternGroups = {
    'Simple patterns (should be ~10x faster with depth limiting)': ['*.js', '*.ts', '*.txt'],
    'Recursive patterns (target: 10-15x faster)': ['**/*.js', '**/*.ts', '**/*'],
    'Scoped patterns (target: 10-15x with prefix walking)': [
      'level0/**/*.js',
      'level0/level1/**/*.ts',
      '**/level1/**/*.ts',
    ],
    'Brace expansion patterns': ['**/*.{js,ts}', 'level{0,1}/**/*.js'],
    'Character class patterns': ['**/*[0-9].js', '**/file[0-9][0-9].ts'],
    'Complex patterns': ['**/level*/**/*.js', './**/level0/**/level1/**/*.js', '**/*/**/*/**/*.js'],
  }
  ...
  if (existsSync(largeFixture)) {
    ...
    const largePatterns = ['*.js', '**/*.js', 'level0/**/*.js', '**/*.{js,ts}', '**/level*/**/*.js']
