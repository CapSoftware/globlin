...
function bench(name: string, fn: () => unknown) {
  ...
  console.log(
    `${name.padEnd(25)}: ${avg.toFixed(2)}ms (min: ${min.toFixed(2)}ms, ${count} results)`
  )
  return avg
}
...
const parallelL = bench('globlin (parallel)', () =>
  globSync(PATTERN, { cwd: LARGE_CWD, parallel: true })
)
const cacheL = bench('globlin (cache warm)', () =>
