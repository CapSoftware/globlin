...
async function main() {
  ...
  for (const [fixtureSize, fixturePath] of Object.entries(FIXTURES)) {
    ...
    for (const { name, pattern } of PATTERNS) {
      try {
        ...
        console.log(
          `${name.padEnd(20)} | globlin: ${globlinTime.toFixed(2).padStart(8)}ms | glob: ${globTime.toFixed(2).padStart(8)}ms | speedup: ${speedup.toFixed(2).padStart(6)}x | match: ${match ? '✓' : '✗'}`
        )
      } catch (error) {
  ...
  for (const fixtureSize of Object.keys(FIXTURES)) {
    ...
    console.log(
      `  Results match:   ${allMatch ? '✓ All patterns match glob' : '✗ Some mismatches'}`
    )
  }
