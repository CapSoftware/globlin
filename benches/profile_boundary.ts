...
function measureRawNative(
  pattern: string,
  cwd: string,
  iterations: number
): { time: number; results: string[] } {
  // Warmup
...
async function measureGlob(
  pattern: string,
  cwd: string,
  iterations: number
): Promise<{ time: number; results: string[] }> {
  // Warmup
...
async function measureSerializationOverhead(): Promise<void> {
  ...
  for (const m of measurements) {
    console.log(
      m.fixture.padEnd(12) +
        ...
        ' |' +
        (m.msPerResult * 1000).toFixed(3).padStart(10) +
        'us'
    )
  }
  ...
  for (const f of fixturesWithCounts) {
    ...
    console.log(
      f.name.padEnd(12) +
        '|' +
        rawResult.time.toFixed(2).padStart(10) +
        'ms |' +
        globlinResult.time.toFixed(2).padStart(10) +
        'ms |' +
        overhead.toFixed(2).padStart(10) +
        'ms |' +
...
async function profileTimingBreakdown(): Promise<void> {
  ...
  for (const fixtureName of Object.keys(FIXTURES)) {
    ...
    const avgRawSpeedup =
      fixtureResults.reduce((a, b) => a + b.rawSpeedup, 0) / fixtureResults.length
    const avgTotalSpeedup =
      fixtureResults.reduce((a, b) => a + b.totalSpeedup, 0) / fixtureResults.length
    const avgOverhead =
      fixtureResults.reduce((a, b) => a + b.jsWrapperPercent, 0) / fixtureResults.length
