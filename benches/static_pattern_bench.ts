...
async function main() {
  ...
  const cwd = await createTestFixture('static-bench', {
    ...
    dirs: ['src', 'src/lib', 'src/components', 'test', 'dist', 'node_modules', '.git'],
  })
  ...
  const dynamicPatterns = ['*.json', 'src/*.ts', '**/*.ts', 'src/**/*.ts']
  ...
  if (staticResults.length > 0) {
    const avgStaticSpeedup =
      staticResults.reduce((a, r) => a + r.speedupVsGlob, 0) / staticResults.length
    const avgStaticVsFg =
      staticResults.reduce((a, r) => a + r.speedupVsFastGlob, 0) / staticResults.length
    console.log(
      `Static patterns: ${avgStaticSpeedup.toFixed(2)}x faster than glob, ${avgStaticVsFg.toFixed(2)}x vs fast-glob`
    )
  }

  if (dynamicResults.length > 0) {
    const avgDynamicSpeedup =
      dynamicResults.reduce((a, r) => a + r.speedupVsGlob, 0) / dynamicResults.length
    const avgDynamicVsFg =
      dynamicResults.reduce((a, r) => a + r.speedupVsFastGlob, 0) / dynamicResults.length
    console.log(
      `Dynamic patterns: ${avgDynamicSpeedup.toFixed(2)}x faster than glob, ${avgDynamicVsFg.toFixed(2)}x vs fast-glob`
    )
  }
