...
async function benchmarkFixture(fixtureName: string): Promise<FixtureSummary> {
  ...
  console.log(`\n${'='.repeat(70)}`)
  console.log(
    `  PARALLEL vs SERIAL BENCHMARK - ${fixtureName} (${fileCount.toLocaleString()} files)`
  )
  ...
  for (const { name, pattern, description } of PATTERNS) {
    ...
    const serialSet = new Set(globSync(pattern, { cwd: fixture, parallel: false }))
    const parallelSet = new Set(globSync(pattern, { cwd: fixture, parallel: true }))
...
function printSummary(summaries: FixtureSummary[]): void {
  ...
  // Overall summary
  const allSpeedups = summaries.flatMap(s => s.patternResults.map(r => r.speedup))
  const overallAvg = allSpeedups.reduce((a, b) => a + b, 0) / allSpeedups.length
  ...
  const mismatches = summaries.flatMap(s => s.patternResults.filter(r => !r.resultsMatch))
