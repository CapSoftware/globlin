...
const pruningPatterns = [
  ...
  {
    pattern: 'src/lib/**/*.ts',
    expectedPruning: 'high',
    note: 'Only needs src/lib/ (~15% of dirs)',
  },
  {
    pattern: 'src/components/**/*.js',
    expectedPruning: 'high',
    note: 'Only needs src/components/',
  },
  ...
  // Patterns with partial prefixes (magic in first segment)
  {
    ...
    expectedPruning: 'medium',
    note: 'src/* is magic - partial pruning',
  },
  ...
  { pattern: '**/*.ts', expectedPruning: 'none', note: 'Globstar at start - no pruning possible' },
  {
    ...
    note: 'Globstar at start - must traverse all',
  },

  // Mixed patterns (prefix + nested globstar)
  {
    pattern: 'src/**/utils/**/*.ts',
    expectedPruning: 'partial',
    note: 'Has prefix but nested globstar',
  },
...
async function main() {
  ...
  console.log(
    `  src/ subtree: ${srcDirs.toLocaleString()} dirs (${((srcDirs / totalDirs) * 100).toFixed(1)}% of total)`
  )
  console.log(
    `  src/lib/ subtree: ${srcLibDirs.toLocaleString()} dirs (${((srcLibDirs / totalDirs) * 100).toFixed(1)}% of total)`
  )
  console.log(
    `  test/ subtree: ${testDirs.toLocaleString()} dirs (${((testDirs / totalDirs) * 100).toFixed(1)}% of total)`
  )
  console.log(
    `  node_modules/ subtree: ${nodeModulesDirs.toLocaleString()} dirs (${((nodeModulesDirs / totalDirs) * 100).toFixed(1)}% of total)`
  )
  ...
  const scopedSpeedup =
    highPruning.length > 0 ? highPruning.reduce((a, r) => a + r.speedup, 0) / highPruning.length : 0
  const nonScopedSpeedup =
    noPruning.length > 0 ? noPruning.reduce((a, r) => a + r.speedup, 0) / noPruning.length : 0
  ...
  if (scopedSpeedup > nonScopedSpeedup * 1.5) {
    console.log(
      `  \x1b[32mPruning is effective!\x1b[0m Scoped patterns are ${(scopedSpeedup / nonScopedSpeedup).toFixed(1)}x faster than non-scoped`
    )
  ...
  console.log(
    `  Expected benefit: src/**/*.ts should only traverse ~${((srcDirs / totalDirs) * 100).toFixed(0)}% of directories`
  )
  console.log('  ' + '='.repeat(70))
