...
async function main() {
  ...
  for (const pattern of patterns) {
    ...
    console.log(
      `  glob:                ${globResult.median.toFixed(2)}ms (${Array.isArray(globResult.results) ? globResult.results.length : 0} results)`
    )
    console.log(
      `  globlin (default):   ${globlinDefaultResult.median.toFixed(2)}ms (${defaultSpeedup.toFixed(2)}x vs glob)`
    )
    console.log(
      `  globlin (nativeIO):  ${globlinNativeResult.median.toFixed(2)}ms (${nativeSpeedup.toFixed(2)}x vs glob, ${nativeVsDefault.toFixed(2)}x vs default)`
    )
    ...
    results.push({
      ...
      nativeSpeedup,
      nativeVsDefault,
      resultCount: Array.isArray(globlinDefaultResult.results)
        ? globlinDefaultResult.results.length
        : 0,
    })
  }
  ...
  console.log('='.repeat(70))
  console.log(
    `${'Pattern'.padEnd(25)} | ${'Results'.padStart(8)} | ${'Default'.padStart(10)} | ${'Native'.padStart(10)} | ${'Native/Def'.padStart(10)}`
  )
  console.log('-'.repeat(70))
