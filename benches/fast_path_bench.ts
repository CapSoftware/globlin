...
const fastPathPatterns = [
  ...
  { pattern: '**/*.{js,ts}', fastPath: 'RecursiveExtensionSet', description: 'Recursive extension set' },
  { pattern: '**/*.{js,ts,txt}', fastPath: 'RecursiveExtensionSet', description: 'Recursive extension set (3)' },
]

// Patterns that require regex (non-fast-path, for comparison)
const regexPatterns = [
  { pattern: '**/*[0-9].js', fastPath: 'None (char class)', description: 'Character class in name' },
  { pattern: '**/file[0-9][0-9].js', fastPath: 'None (char class)', description: 'Multiple char classes' },
  { pattern: '**/file?.js', fastPath: 'None (question)', description: 'Question mark wildcard' },
  { pattern: '**/level*/**/*.js', fastPath: 'None (prefix glob)', description: 'Wildcard in directory' },
]
...
function printTable(title: string, results: BenchResult[]): void {
  ...
  for (const r of results) {
    const patternDisplay = r.pattern.length > 28 
      ? r.pattern.slice(0, 26) + '..' 
      : r.pattern
    ...
    const vsFgStr = r.vsFastGlob >= 1
      ? `${r.vsFastGlob.toFixed(1)}x`
      : `${(1/r.vsFastGlob).toFixed(1)}x-`
    
    const speedupColor = r.speedup >= 3 ? '\x1b[32m' : // green for 3x+
    ...
    console.log(
      ...
      fgColor + vsFgStr.padStart(8) + '\x1b[0m' +
...
async function main() {
  ...
  for (const fixture of fixtures) {
    ...
    printTable('FAST-PATH PATTERNS (extension-only, should use optimized matching):', fastPathResults)
    ...
    console.log(`    Patterns matching: ${fastPathResults.filter(r => r.match).length}/${fastPathResults.length}`)
    console.log('')
    ...
    console.log(`    Patterns matching: ${regexResults.filter(r => r.match).length}/${regexResults.length}`)
