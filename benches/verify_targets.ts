...
async function main() {
  ...
  for (const p of patterns) {
    ...
    console.log(
      `  ${p.padEnd(25)} glob: ${result.glob.toFixed(1)}ms  globlin: ${result.globlin.toFixed(1)}ms  fg: ${result.fastGlob.toFixed(1)}ms  speedup: ${speedup.toFixed(2)}x  vs fg: ${vsFg.toFixed(2)}x`
    )
  }
  console.log(
  ...
  const avgSpeedup =
    (mediumTotal.glob + largeTotal.glob) / (mediumTotal.globlin + largeTotal.globlin)
  ...
  console.log('--------------------------------------------------------------------')
  console.log(
    `Average >= 1.5x faster than glob         ${avgSpeedup.toFixed(2)}x                ${avgSpeedup >= 1.5 ? '✅ PASS' : '❌ FAIL'}`
  )
  console.log(
    `Large fixtures >= 2x faster              ${largeSpeedup.toFixed(2)}x                ${largeSpeedup >= 2.0 ? '✅ PASS' : '❌ FAIL'}`
  )
  console.log(
    `Min: Never slower than glob              ${mediumSpeedup.toFixed(2)}x (medium)     ${neverSlower ? '✅ PASS' : '❌ FAIL'}`
  )
  console.log(
    `All patterns faster on large fixtures    ${largeSpeedup.toFixed(2)}x              ${largeTotal.globlin < largeTotal.glob ? '✅ PASS' : '❌ FAIL'}`
  )
  console.log(
    `Competitive with fast-glob               ${largeVsFg.toFixed(2)}x (vs fg)        ${competitive ? '✅ PASS' : '❌ FAIL'}`
  )
  ...
  if (allTargetsMet) {
    ...
    if (largeSpeedup < 2.0)
      console.log(`  - Large fixture speedup: ${largeSpeedup.toFixed(2)}x (need ≥2x)`)
